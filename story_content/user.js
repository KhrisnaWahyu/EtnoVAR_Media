...
// Menunggu 3 detik (3000ms) untuk memberi waktu
// yang cukup bagi Articulate untuk membuat iframe Web Object.
setTimeout(function() {
  
  // Mengambil SEMUA iframe di DOM
  var iframes = document.getElementsByTagName('iframe');
  var newAllowString = "accelerometer; gyroscope; magnetometer; camera; xr-spatial-tracking; fullscreen";

  for (var i = 0; i < iframes.length; i++) {
    var iframe = iframes[i];

    // Coba deteksi iframe Web Object Assemblr
    // Articulate sering menyimpannya di folder 'WebObjects' atau 'webobject'
    if (iframe.src && iframe.src.includes('WebObjects')) { 
        // Jika ditemukan iframe Articulate yang membungkus web object, tambahkan izin
        iframe.setAttribute('allow', newAllowString);
        iframe.setAttribute('allowfullscreen', 'true');
    }
  }
  
}, 2000); // <-- Penundaan 3 detik
}
